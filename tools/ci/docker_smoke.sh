...
# Substitute: rumor, coworld-rumor, 10.
#
...
#
# env:
#   SMOKE_IMAGE                image, if not given as $1        (coworld-rumor:ci)
#   SMOKE_SLUG                 game slug                        (rumor)
#   SMOKE_GAME_BIN             game entrypoint                  (/bin/rumor)
#   SMOKE_PLAYER_BIN           player entrypoint                (/bin/rumor-player)
#   SMOKE_MANIFEST             manifest template path           (coworld_manifest_template.json)
#   SMOKE_SEATS                seat-count CROSS-CHECK           (10)
...
image="${1:-${SMOKE_IMAGE:-coworld-rumor:ci}}"
slug="${SMOKE_SLUG:-rumor}"
